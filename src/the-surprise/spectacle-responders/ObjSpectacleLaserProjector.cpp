...
void ObjSpectacleLaserProjector::Update(hh::fnd::UpdatingPhase phase, const hh::fnd::SUpdateInfo& updateInfo)
{
    ...
    rotationFrame->SetLocalRotation(Eigen::Quaternionf{ Eigen::AngleAxisf{ rotation, Eigen::Vector3f::UnitY() } });

    size_t laserCount = laserFrames.size();
    for (uint8_t i = 0; i < laserCount; i++) {
        laserFrames[i]->SetLocalRotation(Eigen::Quaternionf{ Eigen::AngleAxisf{i * 6.28319f / static_cast<float>(laserCount), Eigen::Vector3f::UnitY()} *Eigen::AngleAxisf{angle, Eigen::Vector3f::UnitZ() } });
    }
}
